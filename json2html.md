...
**Sample Endpoint:**
```
https://main--demo--amol-anand.aem.live/locations/adobe-locations.json
```
...
2. **`/locations/`** - Dynamic routing with filtering:
   ...
   - Uses `arrayKey: "data"` to specify the array (though in this case the root is already an array)
...
**Note:** The sample JSON has the root as an array, so `arrayKey` might not be needed. However, if your JSON structure is `{ "data": [...] }`, you would use `arrayKey: "data"`.
...
5. Explore advanced features like `relativeURLPrefix` and `useAEMMapping`
